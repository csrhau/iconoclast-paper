...
\noindent
POSE partitions the feasible performance envelope of \autoref{fig:technique} into areas with different performance characteristics.
...
Area 2 encompasses runtime optimisations which dominate $\theta$ in terms of $M$, yet may be outperformed by some power optimised version of $\theta$ (\emph{Weak Runtime Optimisation}).
...
Area 4 corresponds to codes with worse performance than $\theta$ (\emph{Performance Degradation}).
...
The value $t_\theta / t_B$ represents the smallest speed up which guarantees a code that outperformed $\theta$ with respect to $M$.
