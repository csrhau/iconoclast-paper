...
\noindent
We use POSE to investigate the scope for CPU power optimisation for a selection of codes from the Mantevo~\cite{heroux:2009aa} and Rodinia~\cite{che:2009aa} benchmark suites.
...
Prime95 and Linpack were evaluated as $\beta$ benchmarks however they were outperformed by FIRESTARTER.

The benchmark parameter space is small enough to fully characterise a processor by measuring all $(S,A,C)$ configurations.
Benchmarking runs lasted for 120 seconds to allow time for power readings to stabilise.
...
Model summaries for the remaining codes can be found in the Appendix.

These diagram show that LavaMD offers far more scope for power optimisation than MiniMD, which offers virtually none. 
...
\begin{itemize}
  ...
  \item A speed up of 5.92s, or 1.24$\times$, strictly outperforms any power optimisation.
\end{itemize}
...
It would be trivial to use POSE to model optimisations which influence core count; requiring a small change to the parameterisation of the feasible performance envelope ($C_{min}=1$).
