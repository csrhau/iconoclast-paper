...
An important feature of the equations governing power draw is that only $P_{dyn}$ is directly influenced by software, in particular due to its inclusion of the $A$ term. Software can also indirectly effect both dynamic and leakage current if it triggers changes to clock frequency and therefore supply voltage through DVFS. \todo{Ensure define dvfs} 
...
For LavaMD, \autoref{tab:lavamd_pose} gives the longest runtime within the Power Optimized Software Envelope as 69.76s.
...
The lowest value of $Et^2$ within the envelope is 6332608.91, an improvement of 30.59\%.
